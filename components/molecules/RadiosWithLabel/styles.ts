...
export const LabelWrapper = styled.div<{
  ...
  width: ${(props) => {
    ...
    if (props.isSubLabel) return '21.5rem';
...
export const RadiosWrapper = styled.div<{ direction: 'horizontal' | 'vertical' }>`
  display: ${(props) => props.direction === 'horizontal' && 'flex'};
  align-items: ${(props) => props.direction === 'horizontal' && 'center'};
  flex-wrap: wrap;
`;

export const RadioContainer = styled.div<{ direction: 'horizontal' | 'vertical' }>`
  flex-shrink: ${(props) => props.direction === 'horizontal' && 0};
  padding: 0.1875rem 0;
